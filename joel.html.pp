...
◊(define NightSnakeContent (Group (P "Written in Ocaml & C and targetting ASM, Night Snake was my groups final project in the Compilers course at
...
◊; THIS SITE   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
◊(define ThisSiteContent (Group (P "I used this website an excuse to delve back into Racket. I ended up making a mini HTML/CSS compiler which ...
...
◊(define ThisSite (DS ThisSiteContent (TitleSubtitle "This Site" "made with a custom HTML/CSS compiler built in Racket")))

◊; PAGE   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
◊(define Subtitle (P "I build audio tools and compilers.
